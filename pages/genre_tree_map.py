...
def create_treemap2(scrobbles, rank_limit, start_date, end_date):
    records = []
    ...
    artist_counts['rank'] = artist_counts['weight'].rank(method='min', ascending=False)
    # Filter out artists whose rank exceeds rank_limit
    valid_artists = artist_counts[artist_counts['rank'] <= rank_limit]['artist']
    df = df[df['artist'].isin(valid_artists)]

    # Create aggregated dataset for treemap
    artist_genre_counts = df.groupby(['artist', 'genre'])['weight'].sum().reset_index()
    ...
    fig.update_traces(
        hovertemplate='<b>%{label}</b><br>Genre: %{customdata[0]}<br>Plays: %{value}<extra></extra>',
...
def create_treemap(scrobbles, rank_limit, start_date, end_date):
    records = []
    for scrobble in scrobbles:
        artist_name = scrobble["artist_name"]
        genre = scrobble["genre"]
...
def genres():
  ...
  with st.form("rank_compare_form"):
    rank_limit = st.selectbox(
        'Select the Number of Top Artists',
